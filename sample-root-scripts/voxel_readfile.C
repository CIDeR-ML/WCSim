...
int voxel_readfile(const char *filename="../wcsim.root", TString events_tree_name="wcsimrootevent", const int verbose=0)
{
  // Clear global scope
  //gROOT->Reset();
  ...
  TH1F *hvtxR = new TH1F("hvtxR", "Event VTX R;True vertex R (cm);Entries in bin", 20, 5, 25);
  TH1F *hvtxP = new TH1F("hvtxP", "Event VTX #phi;True vertex cos(#phi);Entries in bin", 60, -1.5, 1.5);
  TH1F *hvtxZ = new TH1F("hvtxZ", "Event VTX Z;True vertex Z (cm);Entries in bin", 30, -15, +15);

  TH1F *hdirCosT = new TH1F("hdirCosT", "Event DIR cos(#theta);True direction cos(#theta);Entries in bin", 30, -1.5, 1.5);
  //TH1F *hdirCosT = new TH1F("hdirCosT", "Event DIR cos(#theta);True direction cos(#theta);Entries in bin", 20, -TMath::Pi(), TMath::Pi());
  TH1F *hdirP = new TH1F("hdirP", "Event DIR Phi;True direction #phi (deg);Entries in bin", 54, -90, 450);
  TH1F *hlambda = new TH1F("hlambda", "Photon wavelenghth; Wavelength (nm); Entries in bin", 20, 250, 750);
  ...
  for (long ievent=0; ievent<nevent; ievent++)
  {
    ...
    float VtxX = float(wcsimrootevent->GetVtx(0)*10.);
    float VtxY = float(wcsimrootevent->GetVtx(1)*10.);
    float VtxZ = float(wcsimrootevent->GetVtx(2)*10.);

    float R = sqrt(VtxX*VtxX + VtxZ*VtxZ);
    float cosPhi = VtxX/R;
	...
	continue;
      ...
      float dirX = float(wcsimroottrack->GetDir(0));
      float dirY = float(wcsimroottrack->GetDir(1));
      float dirZ = float(wcsimroottrack->GetDir(2));

      G4ThreeVector dir = G4ThreeVector(dirX, -dirZ, dirY);
      float cosZ = std::cos(dir.theta());
      //float cosZ = dir.theta();
      float dirP = dir.phi()*360./2/TMath::Pi() + 180.;
      
      float eventE = float(wcsimroottrack->GetE());
      float lambda = 1.240E-3/eventE;
      
      //std::cout << "Dir info: " << cosZ << " " << dirP << " " << lambda << std::endl;
  ...
  c1->cd(2); hvtxP->Draw();
  c1->cd(3); hvtxZ->Draw();
  c1->cd(4); hdirCosT->Draw();
  c1->cd(5); hdirP->Draw();
  c1->cd(6); hlambda->Draw();
